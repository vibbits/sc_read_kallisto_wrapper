...
# use config file as argv[0]
#json_path=os.path.abspath('/data/data4GenePattern/test/config.json')
...
for f in sorted(listdir(str(parameter["BASE_DIR"]))):
    ...
    if isfile(join(str(parameter["BASE_DIR"]), f)) and f.startswith(str(parameter['sample_idx'])) and barcode_name == 'R1':
        barcode_filenames.append(f)

read_filenames_seq = [f.replace('R1','R2') for f in barcode_filenames]
read_filenames_umi = barcode_filenames

print "READ FILES:\n"
read_dirs_seq=[]
for i in range(len(read_filenames_seq)):
    read_dirs_seq+=[str(parameter["BASE_DIR"])+read_filenames_seq[i]]
    print read_dirs_seq[i]

print "READ FILES:\n"
read_dirs_umi=[]
for i in range(len(read_filenames_umi)):
    read_dirs_umi+=[str(parameter["BASE_DIR"])+read_filenames_umi[i]]
    print read_dirs_umi[i]
...
command = "cat "
for files in read_dirs_umi:
    command+=files+' '
command+="> "+all_reads_file_umi+".gz"
print "cat..."
os.system(command)
...
os.system("gunzip -f "+all_reads_file_umi+".gz")
...
f1 = open(all_reads_file_umi)

#all_files = [all_reads_file, all_reads_file_umi]

#p=Pool(2)
#ret_vec2 = p.map(file_offset, all_files)
#t1 = time.time()
#print t1-t0, "sec"
#p.close(); p.join()
...
print "number of reads in dataset =",NUM_OF_LINES/4
...
for cell in range(len(codewords)):
    ...
    for i in ret_vec[cell]:
        f.seek(line_offset[i*4])
        temp=f.readline()
        output_check_seq=temp
        f1.seek(line_offset_1[i*4])
        output_check=f1.readline()
